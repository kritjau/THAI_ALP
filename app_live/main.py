...
def _processing_loop():
    ...
    logger.info(
        "ALPR live-only pipeline started (%d camera(s): %s) -- no DB, no JSON export, no saved crops",
...
async def _mjpeg_generator(camera_id: str | None):
    ...
    # unrelated endpoints like /api/cameras. An async generator yields
    # control back to the event loop on every `await` instead.
    while True:
